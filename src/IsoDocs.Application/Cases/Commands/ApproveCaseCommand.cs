...
/// <summary>
/// 核准/結案：核准目前 InProgress 節點；若無下一節點則自動結案。
/// 對應 POST /api/cases/{id}/actions/approve。
/// </summary>
public sealed record ApproveCaseCommand(
    Guid CaseId,
    Guid ActorUserId,
    string? Comment) : ICommand;

public sealed class ApproveCaseCommandValidator : AbstractValidator<ApproveCaseCommand>
{
    public ApproveCaseCommandValidator()
    {
...
public sealed class ApproveCaseCommandHandler : ICommandHandler<ApproveCaseCommand>
{
    private readonly ICaseRepository _cases;
    private readonly IPublisher _publisher;

    public ApproveCaseCommandHandler(ICaseRepository cases, IPublisher publisher)
    {
        _cases = cases;
        _publisher = publisher;
    }

    public async Task<Unit> Handle(ApproveCaseCommand request, CancellationToken cancellationToken)
    {
        ...
        if (@case.Status != CaseStatus.InProgress)
            throw new DomainException("case.invalid_status", "只有進行中的案件可核准");
        ...
        var action = new CaseAction(
            ...
            CaseActionType.Approve, request.ActorUserId, request.Comment, null);
        await _cases.AddActionAsync(action, cancellationToken);
        await _cases.SaveChangesAsync(cancellationToken);

        await _publisher.Publish(
            new CaseActionNotification(@case.Id, currentNode.Id, CaseActionType.Approve, request.ActorUserId),
            cancellationToken);
