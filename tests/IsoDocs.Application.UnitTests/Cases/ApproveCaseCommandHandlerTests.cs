...
public class ApproveCaseCommandHandlerTests
{
    ...
    [Fact]
    public async Task Approve_Last_Node_Should_Close_Case()
    {
        ...
        var handler = new ApproveCaseCommandHandler(repo, publisher);

        await handler.Handle(new ApproveCaseCommand(@case.Id, Guid.NewGuid(), "核准"), CancellationToken.None);

        node.Status.Should().Be(CaseNodeStatus.Completed);
        @case.Status.Should().Be(CaseStatus.Closed);
        repo.Actions.Should().ContainSingle(a => a.ActionType == CaseActionType.Approve);
    }

    [Fact]
    public async Task Approve_With_Next_Node_Should_Not_Close_Case()
    {
        var repo = new FakeCaseRepository();
        var @case = repo.SeedCase(CreateCase());
        ...
        await handler.Handle(new ApproveCaseCommand(@case.Id, Guid.NewGuid(), null), CancellationToken.None);

        @case.Status.Should().Be(CaseStatus.InProgress);
    }

    [Fact]
    public async Task Approve_Should_Publish_Notification()
    {
        var repo = new FakeCaseRepository();
        var @case = repo.SeedCase(CreateCase());
        repo.SeedNode(@case.Id, CreateInProgressNode(@case.Id));
        var publisher = Substitute.For<IPublisher>();
        var handler = new ApproveCaseCommandHandler(repo, publisher);
        var actorId = Guid.NewGuid();

        await handler.Handle(new ApproveCaseCommand(@case.Id, actorId, null), CancellationToken.None);

        await publisher.Received(1).Publish(
            Arg.Is<CaseActionNotification>(n =>
                n.CaseId == @case.Id && n.ActionType == CaseActionType.Approve && n.ActorUserId == actorId),
